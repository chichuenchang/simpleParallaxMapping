...
void BufferIndexedVerts(MeshData& meshdata)
{
   ...
   if (meshdata.mVboTangents != -1)
   {
      glDeleteBuffers(1, &meshdata.mVboTangents);
   }

   if (meshdata.mVboBitangents != -1)
   {
      glDeleteBuffers(1, &meshdata.mVboBitangents);
   }
   ...
   //Buffer vertices
   {
      ...
      glVertexAttribPointer(AttribLoc::Pos, 3, GL_FLOAT, 0, 0, 0);
   }
   ...
   // buffer texture coordinates
   {
      ...
      glVertexAttribPointer(AttribLoc::TexCoord, 2, GL_FLOAT, 0, 0, 0);
   }
   ...
   //buffer normals
   {
      ...
      glVertexAttribPointer(AttribLoc::Normal, 3, GL_FLOAT, 0, 0, 0);
   }

   //buffer tangents
   {
      glGenBuffers(1, &meshdata.mVboTangents);
      glBindBuffer(GL_ARRAY_BUFFER, meshdata.mVboTangents);
      glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * totalNumVerts, 0, GL_STATIC_DRAW);
      ...
      {
         aiMesh* mesh = meshdata.mScene->mMeshes[m];
         std::vector<float> tangents(3 * mesh->mNumVertices);
         if (mesh->HasTangentsAndBitangents())
         {
            for (unsigned int k = 0; k < mesh->mNumVertices; ++k)
            {
               tangents[k * 3] = mesh->mTangents[k].x;
               tangents[k * 3 + 1] = mesh->mTangents[k].y;
               tangents[k * 3 + 2] = mesh->mTangents[k].z;
            }
         }
         const int size = 3 * sizeof(float)*mesh->mNumVertices;
         const void* data = tangents.data();
         glBufferSubData(GL_ARRAY_BUFFER, offset, size, data);
         offset += size;
      }
      glEnableVertexAttribArray(AttribLoc::Tangent);
      glVertexAttribPointer(AttribLoc::Tangent, 3, GL_FLOAT, 0, 0, 0);
   }

   //buffer bitangents
   {
      glGenBuffers(1, &meshdata.mVboBitangents);
      glBindBuffer(GL_ARRAY_BUFFER, meshdata.mVboBitangents);
      glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * totalNumVerts, 0, GL_STATIC_DRAW);
      ...
      {
         aiMesh* mesh = meshdata.mScene->mMeshes[m];
         std::vector<float> bitangents(3 * mesh->mNumVertices);
         if (mesh->HasTangentsAndBitangents())
         {
            for (unsigned int k = 0; k < mesh->mNumVertices; ++k)
            {
               bitangents[k * 3] = mesh->mBitangents[k].x;
               bitangents[k * 3 + 1] = mesh->mBitangents[k].y;
               bitangents[k * 3 + 2] = mesh->mBitangents[k].z;
            }
         }
         const int size = 3 * sizeof(float)*mesh->mNumVertices;
         const void* data = bitangents.data();
         glBufferSubData(GL_ARRAY_BUFFER, offset, size, data);
         offset += size;
      }
      glEnableVertexAttribArray(AttribLoc::Bitangent);
      glVertexAttribPointer(AttribLoc::Bitangent, 3, GL_FLOAT, 0, 0, 0);
   }
